...
const StockOut = () => {
  ...
  const [reason, setReason] = useState("Stock out");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await StockService.record({
        ...
        movement_type: "out",
        quantity: qty,
        reason: reason.trim() || "Stock out",
      });
      notify.success("Stock out recorded.");
      setQuantity("1");
    } catch (error) {
      const err = error as AxiosError<{ message?: string }>;
      notify.error(err.response?.data?.message || "Unable to record stock out.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const content = (
    <div className="max-w-lg space-y-6 pb-8">
      <div>
        <h1 className="text-2xl font-bold text-text">Stock Out</h1>
        <p className="text-sm text-text-muted mt-1">Remove inventory for sales, damage, or consumption.</p>
      </div>
